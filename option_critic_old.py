...
class OptionCriticNet(nn.Module):
    ...
    def get_term_prob(self, feature, option):
        return self.options_term_prob(feature)[:, option].sigmoid()

    def sample_term_prob(self, feature, option):
        return torch.distributions.Bernoulli(self.get_term_prob(feature, option)).sample()
...
class TensorboardLogger():
    def __init__(self, log_folder):
        self.tf_logger = SummaryWriter(log_folder)
    ...
    def log_step(self, step, reward, entropy, loss, actor_loss, critic_loss):
        if step % self.log_interval == 0:
            if loss is not None:
                self.tf_logger.add_scalar('train/loss', loss.item(), step)
                self.tf_logger.add_scalar('train/actor_loss', actor_loss.item(), step)
            if critic_loss is not None:
                self.tf_logger.add_scalar('train/critic_loss', critic_loss.item(), step)
            self.tf_logger.add_scalar('train/entropy', entropy, step)
            self.tf_logger.add_scalar('train/rewards', reward, step)

    def log_episode(self, ep_count, ep_step, ep_reward):
        if ep_count % self.log_interval == 0:
            self.tf_logger.add_scalar('train/episode_steps', ep_step, ep_count)
            self.tf_logger.add_scalar('train/rewards', ep_reward, ep_count)
...
class OptionCritic():
    ...
    def predict(observation, previous_option=0, will_option_terminate=True, deterministic=False):
        feature   = self.option_critic.get_state_feature(self.conv_obs_to_tensor(obs)) # pass through conv network
        if will_option_terminate:
            a_dist = self.option_critic.get_options_q(feature)
        else:
            current_option=previous_option
        if deterministic:
            action = pred_q.argmax(dim=-1)
        else:
            a_dist = torch.distributions.Categorical(a_dist)
            a = a_dist.sample()
    ...
    def learn(self, total_timesteps, log_interval=4, eval_freq=- 1, n_eval_episodes=5, tb_log_name='DQN', eval_log_path=None):
        ...
        while steps < total_timesteps: # training loop
            ...
            obs       = self.env.reset()
            feature   = self.option_critic.get_state_feature(self.conv_obs_to_tensor(obs)) # pass through conv network
            current_option, will_option_terminate = 0, True
            greedy_option = self.option_critic.get_greedy_options(feature)
            ep_reward = 0
            ep_step = 0
            while not game_over:
                ...
                will_option_terminate = self.option_critic.sample_term_prob(feature, current_option) # determine whether or not it ends
                greedy_option = self.option_critic.get_greedy_options(feature)
                ...
                obs = next_obs
                # logging stuff
                self.logger.log_step(steps, reward, entropy.item(), loss, actor_loss, critic_loss)
            ep_count += 1
            self.logger.log_episode(ep_count, ep_step, ep_reward)
