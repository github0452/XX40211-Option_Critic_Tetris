...
def make_env(board_size, seed=0):
    def _init():
        env = TetrisEnv(board_size=board_size, grouped_actions=True, only_squares=True, no_rotations=True, max_steps=500)
        return env
    return _init
...
env = TetrisEnv(board_size=size, grouped_actions=True, only_squares=True, no_rotations=True, max_steps=500)
eval_env = Monitor(TetrisEnv(board_size=size, grouped_actions=True, only_squares=True, no_rotations=True, max_steps=500))
...
model = OptionCritic(env, tensorboard_log=folder, learning_starts=50)
